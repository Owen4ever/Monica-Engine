...
public class NumberProperty extends AbstractProperty<Double> {
    ...
    protected static final EngineThreadGroup TG_PROPERTY_NUM = new EngineThreadGroup("NumberProperty Thread Group");
    ...
    private final class BuffRunnable implements Comparable<BuffRunnable>, Runnable {
            ...
            if (effect.isInterval()) {
                ...
                while (CoreEngine.isStart()) {
                    ...
                    while (CoreEngine.isContinuing()) {
                        ...
                        lock();
                        ((IntervalBuffEffect<Double>) effect).getIntervalEffector().intervalChange();
                        unlock();
                    }
                }
            } else {
        ...
        public int compareTo(BuffRunnable r) {
            if (r == null)
                throw new NullPointerException("Compare with a null NumberBuffRunnable.");
    ...
    private final class LTRunnable implements Comparable<LTRunnable>, Runnable {
        ...
        @Override
        public void run() {
            LongTimeEffect<Double> effect = (LongTimeEffect<Double>) effects.get(pointer);
            int ce1mscc = CoreEngine.get1msSuspendTimeChangeCount();
            int it = CoreEngine.calcSuspendTime(effect.getIntervalDuration());
            while (CoreEngine.isStart()) {
                if (CoreEngine.is1msSuspendTimeChanged(ce1mscc))
                ...
                while (CoreEngine.isContinuing()) {
                    ...
                    if (already == effect.getIntervalDuration())
                        ((IntervalLongTimeEffect<Double>) effect).getIntervalEffector().intervalChange();
                }
            }
        }
        ...
        public int compareTo(LTRunnable r) {
            if (r == null)
                throw new NullPointerException("Compare with a null NumberLongTimeRunnable.");
            return Integer.compare(pointer.pointer(), r.pointer.pointer());
        }
        private boolean isInStartingTime;
        private int startingTimeAlready;
