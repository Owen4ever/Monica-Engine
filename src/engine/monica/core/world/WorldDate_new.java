...
public final class WorldDate_new {

    public WorldDate_new() {
        this(0, l -> l + 1, () -> 1);
    }

    public WorldDate_new(long time, TimeCalc calc, SleepTime sleepTime) {
        if (time < 0)
    ...
    public void setSleepTimeSupplier(SleepTime sleepTime) {
        if (sleepTime == null)
            throw new WorldDateInitializeException("The sleep time supplier is null.");
        this.sleepTime = sleepTime;
    }
    ...
    private final transient EngineThread timer = new EngineThread(new EngineThreadGroup("WorldDate#" + hashCode()),
            () -> {
                while (isStart) {
                    time = calc.calc(time);
                    EngineThread.sleepWithoutException(sleepTime.getSleepTime());
                }
            }, "Timer");
